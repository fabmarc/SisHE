...
public class BancoHorasJdbcDaoImpl extends NamedParameterJdbcDaoSupport implements BancoHorasDAO {
	...
	public List<HistoricoDetalhes> contabilizarHorasBanco(List<Long> idsSolicitacoes) {
		...
		for (Long id : idsSolicitacoes) {
			...
			if (porcentagemFeriado < 0) {
				...
				minutoTotal = ((horaFimSolicitacao * 60) + minutoFimSolicitacao)
				...
				if (minutoTotal > 0) {
					...
					historicoDetalhes.add(new HistoricoDetalhes(minutoTotal, 0, minutoTotal, new Historico(
							new Solicitacao(id))));
				}
			} else {
	...
	public BancoHoras findByUsuario(Usuario entity) {
		...
		BancoHoras banco = getNamedParameterJdbcTemplate().queryForObject(sql.toString(), params,
				new RowMapper<BancoHoras>() {
					@Override
