...
public class UsuarioJdbcDaoImpl extends NamedParameterJdbcDaoSupport implements UsuarioDAO {
	...
	public Usuario update(Usuario entity) throws RegistroInexistenteException {
		int rows = getJdbcTemplate().update("UPDATE usuario SET id_cargo = ?, id_sindicato = ?, id_cidade = ?, nome = ?, login = ?, senha = ?, matricula = ?, email = ? " + "WHERE id = ?", entity.getCargo().getId(), entity.getSindicato().getId(), entity.getCidade().getId(), entity.getNome(),
	...
	public List<Usuario> findAll() {
		return getJdbcTemplate().query("SELECT id, id_cargo as cargo, id_sindicato as sindicato, id_cidade as cidade, nome, login, senha, matricula, email " + "FROM usuario", new BeanPropertyRowMapper<Usuario>(Usuario.class));
	}

	@Override
	public Usuario findById(Object id) throws RegistroInexistenteException {
		try {
			return getJdbcTemplate().queryForObject("SELECT id, id_cargo as cargo, id_sindicato as sindicato, id_cidade as cidade, nome, login, senha, matricula, email " + "FROM usuario WHERE id = ?", new Object[] { id }, new BeanPropertyRowMapper<Usuario>(Usuario.class));
